...
def lambda_handler(event, context):
    ...
    # Initialize the nileBldg class with the API key and tenant ID
    nileBldgs = nileBldg(api_key=api_key, tenant_id=tenant_id)

    try:
        bldgs = nileBldgs.get_buildings()

        return {
            'statusCode': 200,
            'headers': cors_headers,
            'body': json.dumps(bldgs)
        }
