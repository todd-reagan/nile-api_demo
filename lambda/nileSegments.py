...
class nileSegment:
    ...
    def __init__(self, api_key=None, tenant_id=None):
        """
        Initialize the nileSegment class.
    ...
    def get_segments(self):
        ...
        try:
            ...
            if not result:
                raise Exception("Could not get Segment Namea. Likely no segments exist.")
...
def lambda_handler(event, context):
    ...
    # Initialize the nileSegment class with the API key and tenant ID
    nileSegments = nileSegment(api_key=api_key, tenant_id=tenant_id)

    try:
        segments = nileSegments.get_segments()
